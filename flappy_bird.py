...
def welcomeScreen():
    ...
    playerx = int(SCREENWIDTH/2.34)
    playery = int((SCREENHEIGHT - GAME_SPRITES['player'].get_height())/2.2)
    ...
    while True:
        for event in pygame.event.get():
            ...
            # if the user presses space or up key, start the game for him
...
def mainGame():
    ...
    while True:
        ...
        crashTest = isCollide(playerx, playery, upperPipes, lowerPipes) # This function will return True if the bird has crashed
        if crashTest:
            # gameOver()
            return
        ...
        if len(myDigts) == 1:
            Xoffset = (SCREENWIDTH - width)/2.09
        if len(myDigts) == 2:
            Xoffset = (SCREENWIDTH - width)/1.8
        if len(myDigts) == 3:
            Xoffset = (SCREENWIDTH - width)/1.58
        
        for digit in myDigts:
            SCREEN.blit(GAME_SPRITES['numbers'][digit], (Xoffset, SCREENHEIGHT * 0.12))
            Xoffset += GAME_SPRITES['numbers'][digit].get_width() - 35
        pygame.display.update()
        FPSCLOCK.tick(FPS)

def isCollide(playerx, playery, upperPipes, lowerPipes):
    if (playery + GAME_SPRITES['player'].get_height() == GROUNDY) or playery > GROUNDY - 25 or playery < 0:
        GAME_SOUNDS['hit'].play()
        return True
    
    for pipe in upperPipes:
        pipeHeight = GAME_SPRITES['pipe'][0].get_height()
        if (playery < pipeHeight + pipe['y'] and abs(playerx - pipe['x']) < GAME_SPRITES['pipe'][0].get_width() - 28.75):
            GAME_SOUNDS['hit'].play()
            return True

    for pipe in lowerPipes:
        if (playery + GAME_SPRITES['player'].get_height() > pipe['y']) and abs(playerx - pipe['x']) < GAME_SPRITES['pipe'][0].get_width() - 28.75:
            GAME_SOUNDS['hit'].play()
            return True
...
def getRandomPipe():
    ...
    y2 = offset + random.randrange(0, int(SCREENHEIGHT - GAME_SPRITES['base'].get_height() - (0.7 * offset)))
...
if __name__ == "__main__":
    ...
    pygame.display.set_caption('Flappy Bird by Kartik')
